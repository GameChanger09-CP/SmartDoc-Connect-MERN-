...
import { formatIST, getFileUrl, forceDownload, DOC_STATUS } from '../../constants';
...
export default function FacultyDashboard() {
  ...
  const filteredDocs = docs.filter(doc => {
      ...
      if (filterStatus === "Pending") return doc.status === DOC_STATUS.WITH_FACULTY;
      if (filterStatus === "Submitted") return doc.status === DOC_STATUS.FACULTY_REPORTED;
      return true;
  });
  ...
  const handleSubmitReport = async (id, file, note) => {
      ...
      try { await api.post(`/api/documents/${id}/dept_submit_report`, formData); alert("Sent!"); fetchDocs(); } 
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-10 flex justify-between items-end gap-4 border-b pb-6">
            ...
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-xs rounded-lg py-2 px-3 border shadow-sm outline-none"><option value="All">All</option><option value="Pending">Pending</option><option value="Submitted">Submitted</option></select>
        </div>

        <div className="space-y-6">
            {filteredDocs.length === 0 && <p className="text-center text-gray-500 italic py-10">No documents in queue.</p>}
                    ...
                    <div className="p-6">
                        {doc.status === DOC_STATUS.WITH_FACULTY && (
                            <div className="flex flex-col gap-4">
                                <input id={`note-${doc._id}`} className="w-full border p-2 rounded text-sm outline-none focus:border-blue-500" placeholder="Add remarks for Dept Admin..." />
                                <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
                                    <div className="w-full lg:w-2/3"><input type="file" id={`report-${doc._id}`} className="block w-full text-sm file:bg-blue-50 border-0 rounded-lg cursor-pointer file:cursor-pointer"/></div>
                                    <div className="flex gap-3 justify-end">
                                        <button onClick={() => handleReturnToDept(doc._id)} className="px-5 py-2.5 rounded-lg font-bold text-sm bg-gray-100 hover:bg-gray-200 transition">Return</button>
                                        <button disabled={isProcessing} onClick={() => handleSubmitReport(doc._id, document.getElementById(`report-${doc._id}`)?.files[0], document.getElementById(`note-${doc._id}`)?.value)} className="px-6 py-2.5 rounded-lg font-bold text-sm text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 transition">Submit</button>
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                {doc.dept_report && (
                                    <div className="flex gap-2">
                                        <a href={getFileUrl(doc.dept_report)} target="_blank" rel="noreferrer" className="text-xs bg-white text-purple-700 px-3 py-1.5 rounded border hover:bg-purple-50 transition">View</a>
                                        <button onClick={() => forceDownload(getFileUrl(doc.dept_report), `${doc.tracking_id}_report`)} className="text-xs bg-green-50 text-green-700 px-3 py-1.5 rounded border hover:bg-green-100 transition">Download</button>
                                    </div>
                                )}
                            </div>
                        )}
